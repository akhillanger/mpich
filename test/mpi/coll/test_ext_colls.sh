...
for algo_id in ${algo_ids}; do
    for kval in ${kvalues}; do
        for segsize in ${segsizes}; do
            env="env=MPIR_CVAR_USE_REDUCE=${algo_id} env=MPIR_CVAR_REDUCE_TREE_KVAL=${kval} env=MPIR_CVAR_REDUCE_TREE_SEGSIZE=${segsize}${nl}"
...
for algo_id in ${algo_ids}; do
    for kval in ${kvalues}; do
        for segsize in ${segsizes}; do
            ...
            ext_coll_tests+="bcastzerotype 10 ${env} env=MPIR_CVAR_BCAST_TREE_SEGSIZE=${segsize}${nl}"
        done
    done
done
# Allreduce tests
