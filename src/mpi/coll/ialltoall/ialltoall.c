...
cvars:
    ...
    - name        : MPIR_CVAR_IALLTOALL_DEVICE_COLLECTIVE
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_Ialltoall will use allow the device to override the
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Ialltoall_inter_sched(const void *sendbuf, int sendcount, MPI_Datatype
        sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype,
        MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    int mpi_errno = MPI_SUCCESS;

    mpi_errno = MPIR_Ialltoall_generic_inter_sched(sendbuf, sendcount,
