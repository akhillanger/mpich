...
cvars:
    - name        : MPIR_CVAR_INHB_ALLTOALL_ALGORITHM_INTRA
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
      ...
      description : >-
        Variable to select inhb_alltoall algorithm
        auto - Internal algorithm selection
        generic - Force generic algorithm

    - name        : MPIR_CVAR_INHB_ALLTOALL_ALGORITHM_INTER
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
    ...
    - name        : MPIR_CVAR_INHB_ALLTOALL_DEVICE_COLLECTIVE
      category    : COLLECTIVE
      type        : boolean
      default     : true
      class       : device
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_inhb_alltoall will use allow the device to override the
        default, MPIR-level collective algorithms. The device still has the
        option to call the MPIR-level algorithms manually.
        If set to false, the device-level inhb_alltoall function will not be
        called.
                                  ...
                                  MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        switch (MPIR_Inhb_alltoall_alg_intra_choice) {
            case MPIR_INHB_ALLTOALL_ALG_INTRA_GENERIC:
                mpi_errno = MPIR_Ineighbor_alltoall_generic_sched(sendbuf, sendcount, sendtype,
                                                          recvbuf, recvcount, recvtype,
                                                          comm_ptr, s);
                break;
            case MPIR_INHB_ALLTOALL_ALG_INTRA_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Ineighbor_alltoall_generic_sched(sendbuf, sendcount, sendtype,
                                                          recvbuf, recvcount, recvtype,
        ...
        switch (MPIR_Inhb_alltoall_alg_inter_choice) {
            case MPIR_INHB_ALLTOALL_ALG_INTER_GENERIC:
                mpi_errno = MPIR_Ineighbor_alltoall_generic_sched(sendbuf, sendcount, sendtype,
                                                          recvbuf, recvcount, recvtype,
                                                          comm_ptr, s);
                break;
            case MPIR_INHB_ALLTOALL_ALG_INTER_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Ineighbor_alltoall_generic_sched(sendbuf, sendcount, sendtype,
                                                          recvbuf, recvcount, recvtype,
...
int MPIR_Ineighbor_alltoall(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype, MPIR_Comm *comm_ptr, MPI_Request *request)
{
    ...
    *request = MPI_REQUEST_NULL;
    ...
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
    mpi_errno = MPID_Ineighbor_alltoall_sched(sendbuf, sendcount, sendtype,
                                              recvbuf, recvcount, recvtype,
                                              comm_ptr, s);
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    if (MPIR_CVAR_BARRIER_DEVICE_COLLECTIVE && MPIR_CVAR_DEVICE_COLLECTIVES) {
        ...
        mpi_errno = MPIR_Ineighbor_alltoall(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm_ptr, request);
    }
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
...
#   ifdef HAVE_ERROR_CHECKING
    {
        mpi_errno = MPIR_Err_create_code(
            ...
            "**mpi_ineighbor_alltoall", "**mpi_ineighbor_alltoall %p %d %D %p %d %D %C %p", sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm, request);
    }
#   endif
