...
cvars:
    - name        : MPIR_CVAR_NHB_ALLTOALLW_ALGORITHM_INTRA
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
      ...
      description : >-
        Variable to select nhb_alltoallw algorithm
        auto - Internal algorithm selection
        generic - Force generic algorithm

    - name        : MPIR_CVAR_NHB_ALLTOALLW_ALGORITHM_INTER
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
    ...
    - name        : MPIR_CVAR_NHB_ALLTOALLW_DEVICE_COLLECTIVE
      category    : COLLECTIVE
      type        : boolean
      default     : true
      class       : device
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_nhb_alltoallw will use allow the device to override the
        default, MPIR-level collective algorithms. The device still has the
        option to call the MPIR-level algorithms manually.
        If set to false, the device-level nhb_alltoallw function will not be
        called.
...
int MPIR_Neighbor_alltoallw(const void *sendbuf, const int sendcounts[], const MPI_Aint sdispls[], const MPI_Datatype sendtypes[], void *recvbuf, const int recvcounts[], const MPI_Aint rdispls[], const MPI_Datatype recvtypes[], MPIR_Comm *comm_ptr)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        switch (MPIR_Nhb_alltoallw_alg_intra_choice) {
            case MPIR_NHB_ALLTOALLW_ALG_INTRA_NB:
                mpi_errno = MPIR_Neighbor_alltoallw_nb(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts, rdispls, recvtypes, comm_ptr);
                break;
            case MPIR_NHB_ALLTOALLW_ALG_INTRA_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Neighbor_alltoallw_nb(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts, rdispls, recvtypes, comm_ptr);
                break;
        ...
        switch (MPIR_Nhb_alltoallw_alg_inter_choice) {
            case MPIR_NHB_ALLTOALLW_ALG_INTER_NB:
                mpi_errno = MPIR_Neighbor_alltoallw_nb(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts, rdispls, recvtypes, comm_ptr);
                break;
            case MPIR_NHB_ALLTOALLW_ALG_INTER_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Neighbor_alltoallw_nb(sendbuf, sendcounts, sdispls, sendtypes, recvbuf, recvcounts, rdispls, recvtypes, comm_ptr);
                break;
