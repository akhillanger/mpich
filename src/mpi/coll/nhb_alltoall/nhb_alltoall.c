...
cvars:
    - name        : MPIR_CVAR_NHB_ALLTOALL_ALGORITHM_INTRA
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
      ...
      description : >-
        Variable to select nhb_alltoall algorithm
        auto - Internal algorithm selection
        generic - Force generic algorithm

    - name        : MPIR_CVAR_NHB_ALLTOALL_ALGORITHM_INTER
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
    ...
    - name        : MPIR_CVAR_NHB_ALLTOALL_DEVICE_COLLECTIVE
      category    : COLLECTIVE
      type        : boolean
      default     : true
      class       : device
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_nhb_alltoall will use allow the device to override the
        default, MPIR-level collective algorithms. The device still has the
        option to call the MPIR-level algorithms manually.
        If set to false, the device-level nhb_alltoall function will not be
        called.
...
int MPIR_Neighbor_alltoall(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, int recvcount, MPI_Datatype recvtype, MPIR_Comm *comm_ptr)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        switch (MPIR_Nhb_alltoall_alg_intra_choice) {
            case MPIR_NHB_ALLTOALL_ALG_INTRA_NB:
                mpi_errno = MPIR_Neighbor_alltoall_nb(sendbuf, sendcount, sendtype,
                                                      recvbuf, recvcount, recvtype,
                                                      comm_ptr);
                break;
            case MPIR_NHB_ALLTOALL_ALG_INTRA_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Neighbor_alltoall_nb(sendbuf, sendcount, sendtype,
                                                      recvbuf, recvcount, recvtype,
        ...
        switch (MPIR_Nhb_alltoall_alg_inter_choice) {
            case MPIR_NHB_ALLTOALL_ALG_INTER_NB:
                mpi_errno = MPIR_Neighbor_alltoall_nb(sendbuf, sendcount, sendtype,
                                                      recvbuf, recvcount, recvtype,
                                                      comm_ptr);
                break;
            case MPIR_NHB_ALLTOALL_ALG_INTER_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Neighbor_alltoall_nb(sendbuf, sendcount, sendtype,
                                                      recvbuf, recvcount, recvtype,
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    if (MPIR_CVAR_BARRIER_DEVICE_COLLECTIVE && MPIR_CVAR_DEVICE_COLLECTIVES) {
        ...
        mpi_errno = MPIR_Neighbor_alltoall(sendbuf, sendcount, sendtype, recvbuf, recvcount, recvtype, comm_ptr);
    }
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
