 ...
 */
#ifndef MPIR_COLL_PIPELINE_UTIL_H_INCLUDED
#define MPIR_COLL_PIPELINE_UTIL_H_INCLUDED

#undef FUNCNAME
#define FUNCNAME MPIR_COLL_calculate_chunk_info
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
MPL_STATIC_INLINE_PREFIX int MPIR_COLL_calculate_chunk_info(int segsize_input, int type_size,
                                                            int count, int *num_chunks,
                                                            int *num_chunks_floor,
                                                            int *chunk_size_floor,
                                                            int *chunk_size_ceil)
{
    ...
    MPL_DBG_MSG_FMT(MPIR_DBG_COLL, VERBOSE,
                    (MPL_DBG_FDEST, "segment size (in number of elements)=%d\n", segsize));
    if (segsize != 0)
...
#endif /* MPIR_COLL_PIPELINE_UTIL_H_INCLUDED */
