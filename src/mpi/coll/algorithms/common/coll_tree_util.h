...
MPIC_INLINE int COLL_ilog(int k, int number)
{
    int i = 1, p = k - 1;
...
MPIC_INLINE int COLL_ipow(int base, int exp)
{
    int result = 1;
...
MPIC_INLINE int COLL_getdigit(int k, int number, int digit)
{
    return (number / COLL_ipow(k, digit)) % k;
}

/* set the number at 'digit'the location in base k representation of 'number' to newdigit */
MPIC_INLINE int COLL_setdigit(int k, int number, int digit, int newdigit)
{
    int res = number;
...
MPIC_INLINE void COLL_tree_add_child(COLL_tree_t * t, int rank)
{
 ...
 *                                                      7
 */
MPIC_INLINE void COLL_tree_kary_init(int rank, int nranks, int k, int root, COLL_tree_t * ct)
{
    int lrank, child;
 ...
 *                                                     3
 */
MPIC_INLINE void COLL_tree_knomial_1_init(int rank, int nranks, int k, int root, COLL_tree_t * ct)
{
 ...
 *                                                     7
 */
MPIC_INLINE void COLL_tree_knomial_2_init(int rank, int nranks, int k, int root, COLL_tree_t * ct)
{
    int lrank, i, j, basek;
    ...
    for (j = basek-1 ; j >= 0  ; j--) {
        if(flip_bit[j] == 1) {
            for (i = k-1 ; i >= 1 ; i--) {
                ...
                MPIC_DBG("child %d i %d basek %d j %d \n", child, i,basek,j);
                if (child < nranks)
