...
/* Generate the tree */
MPIC_INLINE int COLL_tree_init(int rank, int nranks, int tree_type, int k, int root,
                               COLL_tree_t * ct)
{
    ...
    if (tree_type == 0) /* knomial_1 tree */
        ...
        MPIC_DBG("tree_type %d not defined, initializing knomial tree\n", tree_type);
        COLL_tree_knomial_1_init(rank, nranks, k, root, ct);
    }
...
MPIC_INLINE int COLL_tree_dump(int tree_size, int root, int tree_type, int k)
{
                                ...
                                TSP_sched_t * sched, int finalize)
{
    ...
    MPIC_DBG("Scheduling pipelined broadcast on %d ranks, root=%d\n", TSP_size(&comm->tsp_comm),
             root);
                       ...
                       TSP_sched_t * sched, int finalize, bool per_child_buffer)
{
    ...
    int size = mycomm->tree.nranks;
    int rank = mycomm->tree.rank;
    int is_inplace, dtcopy_id=-1, *vtcs, *reduce_id, *recv_id, nvtcs, child_count;
    void *result_buf;
    ...
    MPIC_DBG("num_children: %d, num_ranks: %d, is_root: %d, is_inplace: %d, is_intermediate: %d, lb: %zd\n",
    ...
    child_count = 0; /* used as a temporary counter while iterating over children list */
    ...
    MPIC_DBG("Allocating buffer for receiving data from children\n");
                            ...
                            TSP_sched_t * s, int finalize, int nbuffers)
{
    ...
    if (root == 0 || is_commutative) {
        ...
        size_t lb, extent, type_size;
        int is_contig, is_inplace, fence_id, send_id;
...
MPIC_INLINE int COLL_sched_barrier_tree(int tag, COLL_comm_t * comm, int k, int tree_type,
                                        TSP_sched_t * s)
{
