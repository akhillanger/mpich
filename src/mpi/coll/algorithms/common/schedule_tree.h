                       ...
                       int finalize, int nbuffers)
{
    ...
    if (!is_inplace && tree->parent == -1) {
        rr_id[1] =
            TSP_dtcopy_nb(recvbuf, count, datatype, sendbuf, count, datatype, &s->tsp_sched, 0,
                          NULL);
    }
        ...
        rr_id[1] =
            TSP_dtcopy_nb(result_buf, count, datatype, sendbuf, count, datatype, &s->tsp_sched, 0,
                          NULL);
    }
    ...
    for (i = 0; i < tree->numRanges; i++) {
        for (j = tree->children[i].startRank; j <= tree->children[i].endRank; j++) {
            if (nbuffers == 1) {
                ...
                rr_id[0] = TSP_recv(result_nbuf[buf], count, datatype,
                                    j, tag, &comm->tsp_comm,
                                    &s->tsp_sched, j == tree->children[0].startRank ? 0 : 1,
                                    &rr_id[2]);
            }
            if (is_commutative) {
                ...
                if (tree->parent == -1)
                    rr_id[2] = TSP_reduce_local(result_nbuf[buf], recvbuf, count,
                                                datatype, op, &s->tsp_sched, is_inplace ? 1 : 2,
                                                rr_id);
                else
                ...
                if (tree->parent == -1) {
                    rr_id[2] = TSP_reduce_local(recvbuf, result_nbuf[buf], count,
                                                datatype, op, &s->tsp_sched, is_inplace ? 1 : 2,
                                                rr_id);
                    TSP_dtcopy_nb(recvbuf, count, datatype, result_nbuf[buf], count, datatype,
                                  &s->tsp_sched, 1, &rr_id[2]);
                }
                else {
                    rr_id[2] = TSP_reduce_local(result_buf, result_nbuf[buf], count,
                                                datatype, op, &s->tsp_sched, 2, rr_id);
                    TSP_dtcopy_nb(result_buf, count, datatype, result_nbuf[buf], count, datatype,
                                  &s->tsp_sched, 1, &rr_id[2]);
                }
            }
            if (nbuffers == 1)
