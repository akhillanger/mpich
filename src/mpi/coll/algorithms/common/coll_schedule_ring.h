                         ...
                         int            finalize)
{
    ...
    MPIC_DBG("rank =%d, data_buf=%04x, sendbuf=%04x, buf1=%04x, buf2=%04x, recvbuf=%04x \n", rank, data_buf, sendbuf, buf1, buf2, recvbuf);

    if (!is_inplace) { /* copy my part of my sendbuf to recv_buf */
        MPIC_DBG("copying my part from sendbuf address=%04x to my recvbuf address=%04x\n", 
    ...
    for(i=0; i<comm_size-1; i++){
        MPIC_DBG ("scheduling phase=%d\n", i);
        
        /* schedule send */       
        MPIC_DBG ("posting send\n");
        nvtcs = 1;
        if (i==0)
            vtcs[0] = dtcopy_id;
        else 
        ...
        /* schedule recv */       
        MPIC_DBG ("posting receive\n");
        MPIC_DBG ("posting recv at address=%04x, count=%d\n", rbuf, comm_size*recvcount);
        if (i==0) nvtcs=0;
...
MPIC_INLINE int COLL_sched_bcast_ring_pipelined (const void * buffer, int count, COLL_dt_t datatype, 
