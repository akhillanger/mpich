...
cvars:
    ...
    - name        : MPIR_CVAR_IREDUCE_DEVICE_COLLECTIVE
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_Ireduce will use allow the device to override the
...
#undef FUNCNAME
#define FUNCNAME ireduce_smp_sched
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
static int ireduce_smp_sched(const void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op, int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
...
int MPIR_Ireduce_sched(const void *sendbuf, void *recvbuf, int count, MPI_Datatype datatype, MPI_Op op, int root, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        if (comm_ptr->hierarchy_kind == MPIR_COMM_HIERARCHY_KIND__PARENT) {
            mpi_errno = ireduce_smp_sched(sendbuf, recvbuf, count,
                        datatype, op, root, comm_ptr, s);
        } else {
            /* intracommunicator */
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    if (MPIR_CVAR_IREDUCE_DEVICE_COLLECTIVE && MPIR_CVAR_DEVICE_COLLECTIVES) {
        mpi_errno = MPID_Ireduce(sendbuf, recvbuf, count, datatype, op, root,
                    comm_ptr, request);
    } else {
        mpi_errno = MPIR_Ireduce(sendbuf, recvbuf, count, datatype, op, root,
                    comm_ptr, request);
    }
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
