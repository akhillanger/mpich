...
cvars:
    - name        : MPIR_CVAR_NHB_ALLGATHERV_ALGORITHM_INTRA
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
      ...
      description : >-
        Variable to select nhb_allgatherv algorithm
        auto - Internal algorithm selection
        generic - Force generic algorithm

    - name        : MPIR_CVAR_NHB_ALLGATHERV_ALGORITHM_INTER
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
    ...
    - name        : MPIR_CVAR_NHB_ALLGATHERV_DEVICE_COLLECTIVE
      category    : COLLECTIVE
      type        : boolean
      default     : true
      class       : device
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_nhb_allgatherv will use allow the device to override the
        default, MPIR-level collective algorithms. The device still has the
        option to call the MPIR-level algorithms manually.
        If set to false, the device-level nhb_allgatherv function will not be
        called.
...
int MPIR_Neighbor_allgatherv(const void *sendbuf, int sendcount, MPI_Datatype sendtype, void *recvbuf, const int recvcounts[], const int displs[], MPI_Datatype recvtype, MPIR_Comm *comm_ptr)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        switch (MPIR_Nhb_allgatherv_alg_intra_choice) {
            case MPIR_NHB_ALLGATHERV_ALG_INTRA_NB:
                mpi_errno = MPIR_Neighbor_allgatherv_nb(sendbuf, sendcount, sendtype,
                                            recvbuf, recvcounts, displs, recvtype,
                                            comm_ptr);
                break;
            case MPIR_NHB_ALLGATHERV_ALG_INTRA_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Neighbor_allgatherv_nb(sendbuf, sendcount, sendtype,
                                            recvbuf, recvcounts, displs, recvtype,
        ...
        switch (MPIR_Nhb_allgatherv_alg_inter_choice) {
            case MPIR_NHB_ALLGATHERV_ALG_INTER_NB:
                mpi_errno = MPIR_Neighbor_allgatherv_nb(sendbuf, sendcount, sendtype,
                                            recvbuf, recvcounts, displs, recvtype,
                                            comm_ptr);
                break;
            case MPIR_NHB_ALLGATHERV_ALG_INTER_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Neighbor_allgatherv_nb(sendbuf, sendcount, sendtype,
                                            recvbuf, recvcounts, displs, recvtype,
