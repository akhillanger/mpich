...
cvars:
    - name        : MPIR_CVAR_IALLGATHER_ALGORITHM_INTRA
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
    ...
    - name        : MPIR_CVAR_IALLGATHER_ALGORITHM_INTER
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
...
#undef FCNAME
#define FCNAME MPL_QUOTE(FUNCNAME)
int MPIR_Iallgather_inter_sched(const void *sendbuf, int sendcount,
        MPI_Datatype sendtype, void *recvbuf, int recvcount,
        MPI_Datatype recvtype, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    int mpi_errno = MPI_SUCCESS;

    mpi_errno = MPIR_Iallgather_generic_inter_sched(sendbuf, sendcount,
                          ...
                          MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
       /* intracommunicator */
        switch (MPIR_Iallgather_alg_intra_choice) {
            case MPIR_IALLGATHER_ALG_INTRA_BRUCKS:
                mpi_errno = MPIR_Iallgather_bruck_sched(sendbuf, sendcount, sendtype,
                            recvbuf, recvcount, recvtype, comm_ptr, s);
                break;
            case MPIR_IALLGATHER_ALG_INTRA_RECURSIVE_DOUBLING:
                mpi_errno = MPIR_Iallgather_rec_dbl_sched(sendbuf, sendcount,
                            sendtype, recvbuf, recvcount, recvtype, comm_ptr, s);
                break;
            case MPIR_IALLGATHER_ALG_INTRA_RING:
                mpi_errno = MPIR_Iallgather_ring_sched(sendbuf, sendcount, sendtype, recvbuf,
                            recvcount, recvtype, comm_ptr, s);
                break;
            case MPIR_IALLGATHER_ALG_INTRA_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Iallgather_intra_sched(sendbuf, sendcount, sendtype, recvbuf,
                            recvcount, recvtype, comm_ptr, s);
                break;
        }
    } else {
        /* intercommunicator */
        switch (MPIR_Iallgather_alg_inter_choice) {
            case MPIR_IALLGATHER_ALG_INTER_GENERIC:
                mpi_errno = MPIR_Iallgather_generic_inter_sched(sendbuf, sendcount, sendtype,
                            recvbuf, recvcount, recvtype, comm_ptr, s);
                break;
            case MPIR_IALLGATHER_ALG_INTER_AUTO:
                MPL_FALLTHROUGH;
            default:
                mpi_errno = MPIR_Iallgather_inter_sched(sendbuf, sendcount, sendtype,
                            recvbuf, recvcount, recvtype, comm_ptr, s);
                break;
        }
    }
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    if (MPIR_CVAR_IALLGATHER_DEVICE_COLLECTIVE && MPIR_CVAR_DEVICE_COLLECTIVES) {
        mpi_errno = MPID_Iallgather(sendbuf, sendcount, sendtype, recvbuf, recvcount,
