...
cvars:
    - name        : MPIR_CVAR_IREDUCE_SCATTER_ALGORITHM_INTRA
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
    ...
    - name        : MPIR_CVAR_IREDUCE_SCATTER_ALGORITHM_INTER
      category    : COLLECTIVE
      type        : string
      default     : auto
      class       : device
    ...
    - name        : MPIR_CVAR_IREDUCE_SCATTER_DEVICE_COLLECTIVE
      ...
      scope       : MPI_T_SCOPE_ALL_EQ
      description : >-
        If set to true, MPI_Ireduce_scatter will use allow the device to override the
                               ...
                               MPI_Datatype datatype, MPI_Op op, MPIR_Comm *comm_ptr, MPIR_Sched_t s)
{
    ...
    if (comm_ptr->comm_kind == MPIR_COMM_KIND__INTRACOMM) {
        /* intracommunicator */
        switch (MPIR_Ireduce_scatter_alg_intra_choice) {
            case MPIR_IREDUCE_SCATTER_ALG_INTRA_NONCOMM:
                mpi_errno = MPIR_Ireduce_scatter_noncomm_sched(sendbuf, recvbuf,
                            recvcounts, datatype, op, comm_ptr, s);
                break;
            case MPIR_IREDUCE_SCATTER_ALG_INTRA_PAIRWISE:
                mpi_errno = MPIR_Ireduce_scatter_pairwise_sched(sendbuf, recvbuf,
                            recvcounts, datatype, op, comm_ptr, s);
                break;
            case MPIR_IREDUCE_SCATTER_ALG_INTRA_RECURSIVE_HALVING:
                mpi_errno = MPIR_Ireduce_scatter_rec_hlv_sched(sendbuf, recvbuf,
                            recvcounts, datatype, op, comm_ptr, s);
                break;
            case MPIR_IREDUCE_SCATTER_ALG_INTRA_RECURSIVE_DOUBLING:
                mpi_errno = MPIR_Ireduce_scatter_rec_dbl_sched(sendbuf, recvbuf,
                            recvcounts, datatype, op, comm_ptr, s);
                break;
            case MPIR_IREDUCE_SCATTER_ALG_INTRA_AUTO:
                MPL_FALLTHROUGH;
            default:
       ...
       /* intercommunicator */
       switch (MPIR_Ireduce_scatter_alg_inter_choice) {
           case MPIR_IREDUCE_SCATTER_ALG_INTER_GENERIC:
               mpi_errno = MPIR_Ireduce_scatter_generic_inter_sched(sendbuf, recvbuf, recvcounts, datatype, op, comm_ptr, s);
               break;
           case MPIR_IREDUCE_SCATTER_ALG_INTER_AUTO:
               MPL_FALLTHROUGH;
           default:
...
#   endif /* HAVE_ERROR_CHECKING */
    ...
    if (MPIR_CVAR_IREDUCE_SCATTER_DEVICE_COLLECTIVE && MPIR_CVAR_DEVICE_COLLECTIVES) {
        mpi_errno = MPID_Ireduce_scatter(sendbuf, recvbuf, recvcounts, datatype, op,
                    comm_ptr, request);
    } else {
        mpi_errno = MPIR_Ireduce_scatter(sendbuf, recvbuf, recvcounts, datatype, op,
                    comm_ptr, request);
    }
    if (mpi_errno) MPIR_ERR_POP(mpi_errno);
