...
MPIC_INLINE int MPIC_MPICH_test(MPIC_MPICH_sched_t * sched);
/* Internal transport function for issuing a vertex in the graph */
MPIC_INLINE void MPIC_MPICH_issue_vtx(int, MPIC_MPICH_vtx_t *, MPIC_MPICH_sched_t *);

/* Transport function for allocating memory */
MPIC_INLINE void *MPIC_MPICH_allocate_mem(size_t size)
{
    return MPL_malloc(size);
}

/* Transport function for free'ing memory */
MPIC_INLINE void MPIC_MPICH_free_mem(void *ptr)
{
...
MPIC_INLINE void MPIC_MPICH_print_schedule (MPIC_MPICH_sched_t *sched) {
  int i, j;
  MPIC_DBG("Collective schedule - \n");
  /* for every vertex in the schedule */
  for (i=0; i<sched->total; i++) {
    MPIC_MPICH_vtx_t *vtx = &sched->vtcs[i];
    MPIC_DBG ("%d(%d) -> ", vtx->id, vtx->kind);
    int num_outvtcs = vtx->outvtcs.used;
    /* print every outgoing vertex */
    for (j=0; j<num_outvtcs; j++) {
      int out_vtx_id = vtx->outvtcs.array[j];
      MPIC_DBG ("%d(%d), ", sched->vtcs[out_vtx_id].id, sched->vtcs[out_vtx_id].kind);
    }
    MPIC_DBG ("\n");
  }
}
...
/* This is set to be static inline because compiler cannot build this function with MPIC_INLINE
                                                                   ...
                                                                   MPIC_MPICH_sched_t * sched)
{
    ...
    MPIC_DBG("Number of outgoing vertices of %d = %d\n", vtxp->id, outvtcs->used);
    ...
    for (i = 0; i < outvtcs->used; i++) {
        ...
        if (num_unfinished_dependencies == 0) {
            MPIC_DBG("Issuing vertex number %d\n", outvtcs->array[i]);
...
MPIC_INLINE void MPIC_MPICH_record_vtx_completion(MPIC_MPICH_vtx_t * vtxp,
                                                  MPIC_MPICH_sched_t * sched)
{
    vtxp->state = MPIC_MPICH_STATE_COMPLETE;
    sched->num_completed++;
    MPIC_DBG("Number of completed vertices = %d\n", sched->num_completed);
...
MPIC_INLINE void MPIC_MPICH_record_vtx_issue(MPIC_MPICH_vtx_t * vtxp, MPIC_MPICH_sched_t * sched)
{
...
MPIC_INLINE int MPIC_MPICH_init()
{
...
MPIC_INLINE void MPIC_MPICH_save_schedule(MPIC_MPICH_comm_t * comm, void *key, int len, void *s)
{
    MPIC_add_sched(&(comm->sched_db), key, len, s);
}

/* Internal transport function to reset issued vertex list */
MPIC_INLINE void MPIC_MPICH_reset_issued_list(MPIC_MPICH_sched_t * sched)
{
    int i, nvtcs;
...
MPIC_INLINE void MPIC_MPICH_sched_init(MPIC_MPICH_sched_t * sched, int tag)
{
    sched->total = 0;
...
MPIC_INLINE void MPIC_MPICH_sched_reset(MPIC_MPICH_sched_t * sched, int tag)
{
    int i;
...
MPIC_INLINE MPIC_MPICH_sched_t *MPIC_MPICH_get_schedule(MPIC_MPICH_comm_t * comm,
                                                        void *key, int len, int tag, int *is_new)
{
    ...
    if (sched) {        /* schedule already exists */
        MPIC_DBG("Schedule is loaded from the database[%p]\n", sched);
        *is_new = 0;
        ...
        *is_new = 1;
        ...
        MPIC_DBG("New schedule is created:[%p]\n", sched);
...
MPIC_INLINE void MPIC_MPICH_reorder_tasks (MPIC_MPICH_int_array *list, MPIC_MPICH_vtx_t* vtcs) {
    int i, j, start_idx, last_idx;
#ifdef MPIC_DEBUG
    MPIC_DBG("List before reordering (task_kind): ");
    for (i=0; i<list->used; i++) {
        MPIC_DBG ("%d ", vtcs[list->array[i]].kind);
    }
    MPIC_DBG ("\n");
#endif
...
#ifdef MPIC_DEBUG
    MPIC_DBG("List after reordering (task_kind): ");
    for (i=0; i<list->used; i++) {
        MPIC_DBG ("%d ", vtcs[list->array[i]].kind);
    }
    MPIC_DBG ("\n");
...
MPIC_INLINE void MPIC_MPICH_add_elems_int_array(MPIC_MPICH_int_array * in, int n_elems, int *elems)
{
    MPIC_DBG("In add_elems_int_array, in->used: %d, in->size: %d\n", in->used, in->size);
    if (in->used + n_elems > in->size) {        /* not sufficient memory */
        int *old_array = in->array;
        int old_array_size = in->size;

        MPIC_DBG ("Increasing size of the int_array\n");
    ...
    MPIC_DBG("Adding %d elems to int_array\n", n_elems);
    /* add new elements */
    memcpy(in->array + in->used, elems, sizeof(int) * n_elems);
    in->used += n_elems;
    MPIC_DBG("Done adding %d elems to int_array\n", n_elems);
}

/* Transport function to tell that the schedule generation is now complete */
MPIC_INLINE void MPIC_MPICH_sched_commit(MPIC_MPICH_sched_t * sched)
{
    int i,j;
...
MPIC_INLINE void MPIC_MPICH_sched_finalize(MPIC_MPICH_sched_t * sched)
{
}

/* Transport function to get commutativity of the operation */
MPIC_INLINE void MPIC_MPICH_opinfo(MPIC_MPICH_op_t op, int *is_commutative)
{
    MPIR_Op *op_ptr;
...
MPIC_INLINE int MPIC_MPICH_isinplace(const void *buf)
{
    if (buf == MPI_IN_PLACE)
        return 1;
...
MPIC_INLINE void MPIC_MPICH_dtinfo(MPIC_MPICH_dt_t dt,
                                   int *iscontig,
...
MPIC_INLINE void MPIC_MPICH_addref_dt(MPIC_MPICH_dt_t dt, int up)
{
...
MPIC_INLINE void MPIC_MPICH_allocate_vtx(MPIC_MPICH_vtx_t * vtx, int invtcs_array_size,
...
MPIC_INLINE void MPIC_MPICH_init_vtx(MPIC_MPICH_sched_t * sched, MPIC_MPICH_vtx_t * vtx, int id)
{
...
MPIC_INLINE void MPIC_MPICH_free_vtx(MPIC_MPICH_vtx_t * vtx)
{
...
MPIC_INLINE void MPIC_MPICH_add_vtx_dependencies(MPIC_MPICH_sched_t * sched, int vtx_id,
...
MPIC_INLINE int MPIC_MPICH_get_new_vtx(MPIC_MPICH_sched_t * sched, MPIC_MPICH_vtx_t ** vtx)
{
...
MPIC_INLINE int MPIC_MPICH_fence(MPIC_MPICH_sched_t * sched)
{
    MPIC_DBG("TSP(mpich) : sched [fence] total=%d \n", sched->total);
...
MPIC_INLINE int MPIC_MPICH_wait_for(MPIC_MPICH_sched_t * sched, int nvtcs, int *vtcs)
{
    MPIC_DBG("TSP(mpich) : sched [wait_for] total=%d \n", sched->total);
    MPIC_MPICH_vtx_t *vtxp;
...
*/
MPIC_INLINE int MPIC_MPICH_wait(MPIC_MPICH_sched_t * sched)
{
    int wait_id;
...
MPIC_INLINE void MPIC_MPICH_addref_op(MPIC_MPICH_op_t op, int up)
{
    MPIR_Op *op_ptr;
...
MPIC_INLINE int MPIC_MPICH_addref_dt_nb(MPIC_MPICH_dt_t dt, int up,
                                        MPIC_MPICH_sched_t * sched, int n_invtcs, int *invtcs)
{
    ...
    MPIC_DBG("TSP(mpich) : sched [%d] [addref_dt]\n", vtx_id);
    return vtx_id;
}

/* Transport function to schedule addition of a reference to a datatype */
MPIC_INLINE int MPIC_MPICH_addref_op_nb(MPIC_MPICH_op_t op, int up,
                                        MPIC_MPICH_sched_t * sched, int n_invtcs, int *invtcs)
{
    /* assign a new vertex */
    MPIC_MPICH_vtx_t *vtxp;
    ...
    MPIC_DBG("TSP(mpich) : sched [%d] [addref_op]\n", vtx_id);
    return vtx_id;
}

/* Transport function to schedule a send task */
MPIC_INLINE int MPIC_MPICH_send(const void *buf,
                                int count,
                                MPIC_MPICH_dt_t dt,
                                int dest,
                                int tag,
                                ...
                                MPIC_MPICH_sched_t * sched, int n_invtcs, int *invtcs)
{
    ...
    MPIC_DBG("TSP(mpich) : sched [%d] [send]\n", vtx_id);
    return vtx_id;
}

MPIC_INLINE int MPIC_MPICH_multicast(const void *buf,
                                     int count,
                                     MPIC_MPICH_dt_t dt,
                                     int* destinations,
...
MPIC_INLINE int MPIC_MPICH_send_accumulate(const void *buf,
                                           int count,
                                           MPIC_MPICH_dt_t dt,
                                           MPIC_MPICH_op_t op,
                                           int dest,
                                           ...
                                           MPIC_MPICH_sched_t * sched, int n_invtcs, int *invtcs)
{
    ...
    MPIC_DBG("TSP(mpich) : sched [%d] [send_accumulate]\n", vtx_id);
    return vtx_id;
}

/* Transport function to schedule a recv task */
MPIC_INLINE int MPIC_MPICH_recv(void *buf,
                                int count,
                                MPIC_MPICH_dt_t dt,
                                int source,
                                int tag,
...
MPIC_INLINE int MPIC_MPICH_queryfcn(void *data, MPI_Status * status)
{
...
MPIC_INLINE int MPIC_MPICH_recv_reduce(void *buf,
                                       int count,
...
MPIC_INLINE int MPIC_MPICH_rank(MPIC_MPICH_comm_t * comm)
{
    return comm->mpid_comm->rank;
}

/* Transport function to get size of the communicator */
MPIC_INLINE int MPIC_MPICH_size(MPIC_MPICH_comm_t * comm)
{
    return comm->mpid_comm->local_size;
}

/* Transport function to schedule a local reduce */
MPIC_INLINE int MPIC_MPICH_reduce_local(const void *inbuf,
                                        void *inoutbuf,
                                        int count,
                                        MPIC_MPICH_dt_t datatype,
                                        ...
                                        MPIC_MPICH_sched_t * sched, int n_invtcs, int *invtcs)
{
    ...
    MPIC_DBG("TSP(mpich) : sched [%d] [reduce_local]\n", vtx_id);
    return vtx_id;
}

/* Transport function to do a blocking data copy */
MPIC_INLINE int MPIC_MPICH_dtcopy(void *tobuf,
                                  int tocount,
...
MPIC_INLINE int MPIC_MPICH_dtcopy_nb(void *tobuf,
                                     int tocount,
                                     MPIC_MPICH_dt_t totype,
                                     const void *frombuf,
                                     int fromcount,
                                     ...
                                     MPIC_MPICH_sched_t * sched, int n_invtcs, int *invtcs)
{
    ...
    MPIC_DBG("TSP(mpich) : sched [%d] [dt_copy]\n", vtx_id);
    return vtx_id;
}

/* Internal transport function to add a single element to MPIC_MPICH_ptr_array */
MPIC_INLINE void MPIC_MPICH_add_elem_ptr_array(MPIC_MPICH_ptr_array * buf_array, void *buf)
{
 ...
 */
MPIC_INLINE void *MPIC_MPICH_allocate_buffer(size_t size, MPIC_MPICH_sched_t * s)
{
...
MPIC_INLINE int MPIC_MPICH_free_mem_nb(void *ptr,
                                       MPIC_MPICH_sched_t * sched, int n_invtcs, int *invtcs)
{
    MPIC_MPICH_vtx_t *vtxp;
    int vtx_id = MPIC_MPICH_get_new_vtx(sched, &vtxp);
    ...
    MPIC_DBG("TSP(mpich) : sched [%d] [free_mem]\n", vtx_id);
    return vtx_id;
}
                                        ...
                                        MPIC_MPICH_sched_t * sched)
{
    ...
    if (vtxp->state == MPIC_MPICH_STATE_INIT && vtxp->num_unfinished_dependencies == 0) {
        ...
        case MPIC_MPICH_KIND_FENCE:
            MPIC_DBG("  --> MPICH transport (fence) complete\n");
            MPIC_MPICH_record_vtx_completion(vtxp, sched);
            break;

        case MPIC_MPICH_KIND_WAIT:
            MPIC_DBG("  --> MPICH transport (wait) complete\n");
            MPIC_MPICH_record_vtx_completion(vtxp, sched);
            break;
        ...
        case MPIC_MPICH_KIND_REDUCE_LOCAL:
            if (vtxp->nbargs.reduce_local.flags == -1 ||
                ...
                MPIC_DBG("Right reduction vtxp->nbargs.reduce_local.flags %ld \n",
                         vtxp->nbargs.reduce_local.flags);
...
MPIC_INLINE int MPIC_MPICH_test(MPIC_MPICH_sched_t * sched)
{
    ...
    if (sched->issued_head == NULL) {
        MPIC_DBG("issued list is empty, issue ready vtcs\n");
        if (sched->total > 0 && sched->num_completed != sched->total) {
            ...
            MPIC_DBG("issued tasks from start_vtcs, sched->total: %d, sched->num_completed: %d\n",
        ...
        if (vtxp->state == MPIC_MPICH_STATE_ISSUED) {
            ...
            case MPIC_MPICH_KIND_MULTICAST:
                for (i=vtxp->nbargs.multicast.last_complete+1; i<vtxp->nbargs.multicast.num_destinations; i++) {
                    if(MPIR_Request_is_complete(vtxp->nbargs.multicast.mpir_req[i])){
                        MPIC_DBG("  --> MPICH transport multicast task %d complete\n", i);
            ...
            case MPIC_MPICH_KIND_RECV_REDUCE:
                ...
                if (mpid_req1 && MPIR_Request_is_complete(mpid_req1)) {
                    ...
                    MPIC_DBG("reduce in recv_reduce also completed\n");
                    vtxp->mpid_req[1] = NULL;
    ...
    if (sched->num_completed == sched->total) {
        MPIC_DBG("  --> MPICH transport (test) complete:  sched->total=%d\n", sched->total);
    }
#endif
...
MPIC_INLINE void MPIC_MPICH_free_buffers(MPIC_MPICH_sched_t * sched)
{
    int i;
    for (i = 0; i < sched->total; i++) {
    ...
    MPIC_DBG("free'd any buffer memory associated with vertices\n");
...
MPIC_INLINE int MPIC_MPICH_comm_cleanup(MPIC_MPICH_comm_t * comm)
{
    MPIC_DBG("In MPIC_MPICH_comm_cleanup for comm: %p\n", comm);
